 ...
 * 1.
 */
public class DetailSingleScrollView extends ViewGroup {

    private static final String TAG = DetailSingleScrollView.class.getSimpleName();
   ...
   // private IDetailListView mListView;
    private IDetailSingleWebView mWebView;
    ...
    public DetailSingleScrollView(Context context) {
        super(context);
        init(context);
    }

    public DetailSingleScrollView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public DetailSingleScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child instanceof IDetailSingleWebView) {
                mWebView = (IDetailSingleWebView) child;
            }
        }
       /* if (mListView != null) {
    ...
    private void observeWebViewContentHeightChange() {
        if (mWebView instanceof DetailSingleWebView) {
           final View view = ((DetailSingleWebView) mWebView).getView();
            view.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {

                private int mOldWebViewContentHeight;
                private int mOldMaxHeight=0;
                ...
                public boolean onPreDraw() {
                    ...
                    int distBottom = DetailSingleScrollView.this.getHeight() - outRect.height();
                    if (distBottom <= 0
                            || DetailSingleScrollView.this.getScrollY() == 0//没触发滑动的时候不扩展
                            || webView.getHeight() < DetailSingleScrollView.this.getHeight())//webView本身高度就小于scrollview，也不扩展
                        return true;
                    ...
                    int maxheight=webView.getMeasuredHeight() + distBottom;
                    if (maxheight > mParentHeight && (mParentHeight != 0)) {
                        maxheight=mParentHeight;
                    }
                    if(mOldMaxHeight==maxheight) return true;
                    mOldMaxHeight=maxheight;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        float y = event.getY();
        boolean isAtTop = getScrollY() >= maxScrollY;//MyScroll是否在头部   对应结束位置
        boolean isAtBottom = getScrollY() <= 0;//MyScroll是否在底部         对应开始位置
...
//        LogE(TAG + ".customScrollBy.......oldY=" + oldY + ",getScrollY()=" + getScrollY());
    ...
    public void computeScroll() {
        ...
        if (currY < oldY && oldY <= 0) {//向下惯性滑动  到顶部之后
            if (curVelocity != 0) {
                ...
                LogE("computeScroll...webView start fling:" + (-curVelocity)+"___"+mMinimumVelocity);
                this.mScroller.forceFinished(true);

                curVelocity=Math.abs(curVelocity)>(3*mMinimumVelocity)? (int)(curVelocity/mScale):curVelocity;

                this.mWebView.startFling(-curVelocity);
                return;
            }                           //向上惯性滑动  到底部之后
        } else if (currY > oldY && oldY >= maxScrollY && curVelocity != 0 &&false ) {//&&mListView.startFling(curVelocity)
    ...
    protected int computeVerticalScrollExtent() {
        try {
            ...
            int listExtent = 0;//ViewUtils.computeVerticalScrollExtent((View) mListView);
    ...
    protected int computeVerticalScrollOffset() {
        try {
            ...
            int listOffset =0;// ViewUtils.computeVerticalScrollOffset((View) mListView);
    ...
    protected int computeVerticalScrollRange() {
        try {
            ...
            int listScrollRange = 0;//ViewUtils.computeVerticalScrollRange((View) mListView);
