...
final class CalendarViewDelegate {
    ...
    CalendarViewDelegate(Context context, @Nullable AttributeSet attrs) {
       ...
       // mWeekStart = array.getInt(R.styleable.CalendarView_week_start_with, WEEK_START_WITH_SUN);
        ...
        mDayTextSize = array.getDimensionPixelSize(R.styleable.CalendarView_day_text_size, CalendarUtil.dipToPx(context, 14));
        mLunarTextSize = array.getDimensionPixelSize(R.styleable.CalendarView_lunar_text_size, CalendarUtil.dipToPx(context, 10));
        mCalendarItemHeight = (int) array.getDimension(R.styleable.CalendarView_calendar_height, CalendarUtil.dipToPx(context, 38));
    ...
    private void init() {
        ...
        //重置年月范围
        int currentYear=CalendarUtil.getDate("yyyy", d);
        int currentMonth=CalendarUtil.getDate("MM", d);
        mMinYear=currentYear-1;
        mMaxYear=currentYear+1;
        mMinYearMonth=currentMonth;
        mMaxYearMonth=currentMonth;

        mCurrentDate.setYear(currentYear);
        mCurrentDate.setMonth(currentMonth);
