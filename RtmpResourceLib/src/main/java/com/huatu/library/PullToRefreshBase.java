...
import com.huatu.library.internal.RepeatCustomLoadingLayout;
import com.huatu.library.internal.RotateImgLoadingLayout;
...
import static com.huatu.library.PullToRefreshBase.State.RESET;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public static final float OVER_PULL_FRICTION=0.7f;
	...
	//修该此处 去掉final  write by xing 2016
	@Override
	...
	public final boolean isRefreshing() {
		return mState == State.REFRESHING || mState == State.MANUAL_REFRESHING||mState== State.CUSTOM;
	}
	...
	public final void onRefreshCustomComplete() {
		if (isRefreshing()) {
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				...
				switch (mCurrentMode) {
					...
					default:
					case PULL_FROM_START:
 						if(mLoadingAnimationStyle== AnimationStyle.FLIP){
 							if(!isOverPull)
								smoothScrollTo(-getHeaderSize()/2, listener);
							else
	...
	protected void onResetCustom() {
		...
		if(mDiffTop==0)mDiffTop= DensityUtils.dp2px(getContext(),32);
		smoothScrollTo(-mDiffTop);
	}
	...
	boolean isOverPull=false;
    //add
	public boolean hasOverPull(){
       return isOverPull;
	}
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			//change
			if(mLoadingAnimationStyle== AnimationStyle.FLIP){
				isOverPull=Math.abs(newScrollValue)>(OVER_PULL_FRICTION*itemDimension);


				if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(2*newScrollValue)) {
					setState(State.PULL_TO_REFRESH);
				} else if (mState == State.PULL_TO_REFRESH && itemDimension < Math.abs(2*newScrollValue)) {
	...
	public static enum AnimationStyle {
		...
		LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
			switch (this) {
				...
				case OLDFLIP:
				case FLIP:
					//return new RotateImgLoadingLayout(context, mode, scrollDirection, attrs,true);
					  return new FlipLoadingLayout(context, mode, scrollDirection, attrs,true);
				case CUSTOM:
					return new RepeatCustomLoadingLayout(context, mode, scrollDirection, attrs,false);
			}
		}
	}
