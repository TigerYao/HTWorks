...
public final class ArrayUtils {
    ...
    public static final <D> List<D> filterList(Collection<D> colls, FilterBy<D> gb) {
        if(ArrayUtils.isEmpty(colls)) return null;
        if (gb == null) {
            System.out.println("分組依據接口不能為Null!");
            return null;
    ...
    public static final boolean isInArrayRange(Collection<?> collection,int postion){
        return (postion>=0)&&(postion<size(collection));
     }
