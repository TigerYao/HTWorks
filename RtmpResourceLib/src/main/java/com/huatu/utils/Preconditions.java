...
/**
 * author : Soulwolf Create by 2015/6/16 14:59
...
public final class Preconditions {
    ...
    public static <T> void checkResponse(T reference){
        if(reference == null){
    ...
    private static String badPositionIndexes(int start, int end, int size) {
        return start >= 0 && start <= size?(end >= 0 && end <= size?format("end_time index (%s) must not be less than start index (%s)", new Object[]{Integer.valueOf(end), Integer.valueOf(start)}):badPositionIndex(end, size, "end_time index")):badPositionIndex(start, size, "start index");
    }
