...
public final class OverscrollHelper {
	 ...
	 */
	public static void overScrollBy(final ScrollFrameLayout<?> view, final int deltaX, final int scrollX,
	...
	/*	switch (view.getPullToRefreshScrollDirection()) {
		...
		if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
			...
			// touch
			if (true && !isTouchEvent && deltaValue != 0) {//mode.permitsPullToRefresh()
			...
			/*	if (PullToRefreshBase.DEBUG) {
				...
				if (newScrollValue < (0 - fuzzyThreshold)) {
					...
					// then move scroll
					if (true) {//mode.showHeaderLoadingLayout()
						// If we're currently at zero, we're about to start
						// overscrolling, so change the state
						if (currentScrollValue == 0) {
 						   view.setState(ScrollFrameLayout.State.OVERSCROLLING);
						}
					...
					// Check the mode supports the overscroll direction, and
					// then move scroll
					if (true) {//mode.showFooterLoadingLayout()
						// If we're currently at zero, we're about to start
						// overscrolling, so change the state
						if (currentScrollValue == 0) {
					...
					view.setState(ScrollFrameLayout.State.RESET);
				}
			} else if (isTouchEvent && ScrollFrameLayout.State.OVERSCROLLING == view.getState()) {
