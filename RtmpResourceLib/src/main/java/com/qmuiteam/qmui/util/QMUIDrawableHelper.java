...
public class QMUIDrawableHelper {
    ...
    public static BitmapDrawable createBordDrawableWithSizeV2(Resources resources, int width, int height, int cornerRadius, @ColorInt int filledColor, @ColorInt int bordColor) {
        Bitmap output = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(output);
        ...
        if(cornerRadius<(width/3)){//
            if (cornerRadius > 0) {
                ...
                // 设置画出的线的 粗细程度
                float borderWidth=resources.getDimension(R.dimen.common_circle_bord);
                paint.setStrokeWidth(borderWidth);
                paint.setColor(bordColor);
                canvas.drawRoundRect(new RectF(borderWidth, borderWidth, width-borderWidth, height-borderWidth), cornerRadius, cornerRadius, paint);
            } else {
                canvas.drawColor(filledColor);
            }
        }else{
            ...
            if (cornerRadius > 0) {
                ...
                paint.setStrokeWidth(borderWidth);
                paint.setColor(bordColor);
                canvas.drawCircle(width/2, width/2, width/2-borderWidth, paint);
            } else {
                canvas.drawColor(filledColor);
            }
        }
    ...
    public static BitmapDrawable createBordDrawableWithSize(Resources resources, int width, int height, int cornerRadius, @ColorInt int filledColor,@ColorInt int bordColor) {
        Bitmap output = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(output);
