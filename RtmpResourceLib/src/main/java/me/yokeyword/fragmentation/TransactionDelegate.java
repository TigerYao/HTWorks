...
class TransactionDelegate {
    ...
    /**
     * Start the target Fragment and pop itself   先移除后添加
     */
        ...
        enqueue(fm, new Action(Action.ACTION_POP_MOCK) {
            ...
            public void run() {
                ...
                if (!FragmentationMagician.isStateSaved(fm)) {
                    //将fromFragment的视图移动到mockView   以及进入动画时移除前一个view
                    mockStartWithPopAnim(SupportHelper.getTopFragment(fm), to, top.getSupportDelegate().mAnimHelper.popExitAnim);
                }

                //
                removeTopFragment(fm);//移除fragment
                FragmentationMagician.popBackStackAllowingStateLoss(fm);//回退栈            fragmentManager.popBackStack();
                FragmentationMagician.executePendingTransactionsAllowingStateLoss(fm);  // fragmentManager.executePendingTransactions();
