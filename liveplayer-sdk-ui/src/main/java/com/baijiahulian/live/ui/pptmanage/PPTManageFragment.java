            ...
            if (holder instanceof DocViewHolder) {
                ...
                if (res == 0) { // pic
                   ...
                   // Picasso.with(getContext()).load(url).into(docViewHolder.ivIcon);
                    ...
                    Glide.with(getContext()).load(url).apply(transbgOptions)
                            .transition(DrawableTransitionOptions.withCrossFade(250)).into(docViewHolder.ivIcon);
                } else {
                    try {
                        docViewHolder.ivIcon.setImageResource(res);
                    }catch (Exception e){}
                ...
                if (res == 0) {
                    ...
                    viewHolder.ivIcon.setImageResource(R.drawable.live_ic_file_jpg);
                } else {
                    // Picasso.with(getContext()).load(res).into(viewHolder.ivIcon);
                    try {
                        viewHolder.ivIcon.setImageResource(res);
                    } catch (Exception e) {
                    }
