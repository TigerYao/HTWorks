...
public class QuickSwitchPPTFragment extends BaseDialogFragment implements SwitchPPTContract.View {
    ...
    private class QuickSwitchPPTAdapter extends RecyclerView.Adapter<SwitchHolder> {
        private   final RequestOptions transbgOptions =
        ...
        public void onBindViewHolder(final SwitchHolder holder, int position) {
            //Picasso.with(getContext()).load(AliCloudImageUtil.getScaledUrl(quickDocList.get(position).url, AliCloudImageUtil.SCALED_MFIT, 200, 200)).into(holder.PPTView);

            Glide.with(getContext()).load(AliCloudImageUtil.getScaledUrl(quickDocList.get(position).url, AliCloudImageUtil.SCALED_MFIT, 200, 200)).apply(transbgOptions)
                    .transition(DrawableTransitionOptions.withCrossFade(250)).into(holder.PPTView);
