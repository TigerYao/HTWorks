...
public class TinkerReport {
    ...
    public static void onTryApplyFail(int errorCode) {
        ...
        switch (errorCode) {
            ...
            case ShareConstants.ERROR_PATCH_JIT:
                ...
                break;
            case TinkerMethods.ERROR_PATCH_ROM_SPACE:
                reporter.onReport(KEY_TRY_APPLY_ROM_SPACE);
                break;
            case TinkerMethods.ERROR_PATCH_GOOGLEPLAY_CHANNEL:
                reporter.onReport(KEY_TRY_APPLY_GOOGLEPLAY);
                break;
            case TinkerMethods.ERROR_PATCH_ALREADY_APPLY:
                reporter.onReport(KEY_TRY_APPLY_ALREADY_APPLY);
                break;
            case TinkerMethods.ERROR_PATCH_CRASH_LIMIT:
                reporter.onReport(KEY_TRY_APPLY_CRASH_LIMIT);
                break;
            case TinkerMethods.ERROR_PATCH_MEMORY_LIMIT:
                reporter.onReport(KEY_TRY_APPLY_MEMORY_LIMIT);
                break;
            case TinkerMethods.ERROR_PATCH_CONDITION_NOT_SATISFIED:
    ...
    public static void onLoaded(long cost) {
        ...
        TinkerLog.e("changxin", "tinkerReort success, report load cost " + cost);
        if (cost <= 500) {
    ...
    public static void onLoadInterpretReport(int type, Throwable e) {
        ...
        switch (type) {
            case ShareConstants.TYPE_INTERPRET_GET_INSTRUCTION_SET_ERROR:
                ...
                reporter.onReport("Tinker Exception:interpret occur exception " + TinkerMethods.getExceptionCauseString(e));
                break;
    ...
    public static void onLoadException(Throwable throwable, int errorCode) {
        ...
        if (!isCheckFail) {
            reporter.onReport("Tinker Exception:load tinker occur exception " + TinkerMethods.getExceptionCauseString(throwable));
        }
    }
    ...
    public static void onApplyDexOptFail(Throwable throwable) {
        ...
        if (throwable.getMessage().contains(ShareConstants.CHECK_DEX_OAT_EXIST_FAIL)) {
            ...
            reporter.onReport("Tinker Exception:apply tinker occur exception " + TinkerMethods.getExceptionCauseString(throwable));
        }
    }
