...
public class ToastUtils {
    ...
    public static void showCenter(final Activity context, final CharSequence message){
        if(Method.isActivityFinished(context)) return;
        ...
        EasySnackBar.make(rootView, R.layout.toast_sys_layout, BaseTransientBar.LENGTH_SHORT,message).show();
    }

    public static void showTop(final Activity context, final CharSequence message){
        if(Method.isActivityFinished(context)) return;
        if (Thread.currentThread() != Looper.getMainLooper().getThread())  return;
        View rootView= context.getWindow().getDecorView().findViewById(android.R.id.content);
        if(rootView==null){
        ...
        EasySnackBar.make(rootView, R.layout.toast_sys_layout, BaseTransientBar.LENGTH_SHORT,true,message).show();
    }

    public static void showBottom(final Activity context, final CharSequence message){
        if(Method.isActivityFinished(context)) return;
        if (Thread.currentThread() != Looper.getMainLooper().getThread())  return;
        ...
        EasySnackBar.make(rootView, R.layout.toast_sys_layout, BaseTransientBar.LENGTH_SHORT,false,message).show();
    }
    ...
    public static void showRewardToast(String key) {
        ...
        showAddCoinTopToast(text);
    }

    public static void showAddCoinTopToast(final String text) {
        UniApplicationLike.getApplicationHandler().post(new Runnable() {
            @Override
            public void run() {
                if (null == UniApplicationContext.getContext()) return;
                Toast curToast = null;
                if (curToast == null) {
                    curToast = new Toast(UniApplicationContext.getContext());
                    View toastView = LayoutInflater.from(UniApplicationContext.getContext())
                            .inflate(R.layout.reward_toast_layout, null, false);
                    curToast.setView(toastView);
                    curToast.setGravity(Gravity.TOP, 0, 0);
                    curToast.setDuration(Toast.LENGTH_LONG);
                }
                if (curToast.getView() != null) {
                    TextView msgView = (TextView) curToast.getView().findViewById(R.id.reward_toast_message_tv);
                    if (msgView != null) {
    ...
    */
    ...
    public static void showShortToast(Context context, CharSequence text, int gravity) {
    ...
    private static Toast setText(Toast curToast, CharSequence text) {
