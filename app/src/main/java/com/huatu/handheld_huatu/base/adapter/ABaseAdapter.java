...
public abstract class ABaseAdapter<T> extends BaseAdapter {
    ...
    public ABaseAdapter(ArrayList<T> datas, Context context,boolean hasStyle){
        this(datas,context);
        mHasStyle=hasStyle;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null || !isReusing) {
            ...
            if(mHasStyle)
                convertView=LayoutInflater.from(context).inflate(itemViewProcessor.inflateViewId(),parent,false);
            else
                convertView = View.inflate(context, itemViewProcessor.inflateViewId(), null);
            convertView.setTag(R.id.reuse_tag,itemViewProcessor);

            itemViewProcessor.bindingView(convertView);
        } else {
            itemViewProcessor = (AbstractItemView<T>) convertView.getTag(R.id.reuse_tag);
        }
