package com.huatu.handheld_huatu.view.swiperecyclerview.swipemenu;
...
public class SwipeMenuLayout extends FrameLayout {
    ...
    public boolean onSwipe(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if ((isFling || Math.abs(mDownX - event.getX()) > (mMenuView.getWidth() / 6)) &&
                        Math.signum(mDownX - event.getX()) == mSwipeDirection) {
                    smoothOpenMenu();
                } else {
                    View pointView = findTopChildUnder(this,mDownX,mDownY);
                    if (pointView!=null&&pointView==mContentView&&this.isOpen()){
                    smoothCloseMenu();
                    return false;
                    }else {
                        return false;
    ...
    private void swipe(int dis) {
        if (Math.signum(dis) != mSwipeDirection||Math.abs(dis)<mMenuView.getWidth()/6) {
            dis = 0;
