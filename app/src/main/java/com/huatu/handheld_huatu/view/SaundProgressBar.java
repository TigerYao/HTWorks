...
public class SaundProgressBar extends ProgressBar {
	...
	public SaundProgressBar(Context context, AttributeSet attrs, int defStyle) {
		...
		if (a != null) {
			m_textPaint.setTextSize(a.getDimension(
					R.styleable.SaundProgressBar_progress_textSize, 10));
			m_textPaint.setColor(a.getColor(
					R.styleable.SaundProgressBar_progress_textColor, Color.WHITE));
			...
			int textStyle = (a
					.getInt(R.styleable.SaundProgressBar_progress_textStyle, 1));
			if (textStyle == 0) {
	 ...
	 * implements the {@linkplain Formatter} interface.
	 * 
	 * @param formatter
	 */
