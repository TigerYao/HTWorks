...
public class CustomViewfinderView extends ViewfinderView {
    ...
    public void onDraw(Canvas canvas) {
        ...
        float lineLength = Math.min(frame.width(), frame.height()) * mLineRate;
        //绘制4个角
        paint.setColor(mLineColor); // 定义画笔的颜色
        canvas.drawRect(frame.left, frame.top, frame.left + lineLength, frame.top + mLineDepth, paint);
        canvas.drawRect(frame.left, frame.top, frame.left + mLineDepth, frame.top + lineLength, paint);

        canvas.drawRect(frame.right - lineLength, frame.top, frame.right+2, frame.top + mLineDepth, paint);
        canvas.drawRect(frame.right - mLineDepth, frame.top, frame.right+2, frame.top + lineLength, paint);

        canvas.drawRect(frame.left, frame.bottom - mLineDepth, frame.left + lineLength, frame.bottom, paint);
        canvas.drawRect(frame.left, frame.bottom - lineLength, frame.left + mLineDepth, frame.bottom, paint);

        canvas.drawRect(frame.right - lineLength, frame.bottom - mLineDepth, frame.right, frame.bottom, paint);
        canvas.drawRect(frame.right - mLineDepth, frame.bottom - lineLength, frame.right, frame.bottom, paint);
