...
import com.huatu.handheld_huatu.business.essay.camera.cropper.util.CamParaUtil;
...
//https://www.cnblogs.com/aspirant/p/7602151.html
//https://www.cnblogs.com/liuling/p/2015-10-28-01.html
...
public class CameraPreview extends SurfaceView implements
    ...
    private CamParaUtil mCamUtils;

    private boolean mHasStopPreview=false;
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        LogUtils.d("zyw","camera is : "+camera);
        boolean openOk = true;
        if (camera == null) {
            //华为 TAG-AL00 CPU型号：MT6753T 系统版本：Android 5.1 camera获取为空，再执行打开
            try {
                camera = Camera.open(); // attempt to get a Camera instance
            } catch (Exception e){
                // Camera is not available (in use or does not exist).
                LogUtils.d("zyw","exception is : "+e.toString());
                openOk = false;
            }
        }
        if (openOk && camera != null) {
            mParams = camera.getParameters();
            mCamUtils = new CamParaUtil(mParams);
            mCamUtils.printSupportPictureSize(mParams);
            mCamUtils.printSupportPreviewSize(mParams);
            try {
            ...
            Toast.makeText(getContext(), "摄像头打开失败！", Toast.LENGTH_LONG).show();
        }
...
//        setFocus();
    ...
    OnTouchListener onTouchListener = new OnTouchListener() {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                int delayMs=(int)((System.currentTimeMillis() - mFocusTouch) );//
                if(delayMs>=200) { //防止点击过快
                    mFocusTouch = System.currentTimeMillis();
                    focusOnTouch(event);
                }
    ...
    private void updateCameraParameters() {
        if (camera != null) {
            Camera.Parameters p = mParams;//camera.getParameters();
...
//                camera.setParameters(p);
                Camera.Size previewSize = mCamUtils.getPropPreviewSize();
                Camera.Size prePicSize =  mCamUtils.getPropPictureSize();
                LogUtils.d("zyw","inexception previewSize w:"+previewSize.width+" ;h:"+previewSize.height);
                LogUtils.d("zyw","inexception prePicSize w:"+prePicSize.width+" ;h:"+prePicSize.height);
                p.setPreviewSize(previewSize.width, previewSize.height);
                p.setPictureSize(prePicSize.width,prePicSize.height);
                camera.setParameters(p);
            }
        }
    }
    ...
    public void focusOnTouch(MotionEvent event) {
        ...
        if((camera==null)||(mFocusView==null)) return;
        ...
        try {
            ...
            camera.autoFocus(this);//https://mobile.umeng.com/platform/54645278fd98c565730006bc/error_types/list/2777843675188
        } catch (Exception e) {
        }
