...
public class ScrollLinearLayout extends LinearLayout {
    ...
    public void smoothScrollBy(int dx, int dy,boolean isExpland) {
        ...
        if(isExpland){
            ...
            mScroller.startScroll(0,curScroll, dx, -curScroll,400);
        }
...
/*
    ...
    private static boolean isRecyclerViewTop(View recyView) {
        if(null==recyView) return true;
        if(recyView instanceof RecyclerViewTopEx){
            return ((RecyclerViewTopEx)recyView).isTop();
        }
        else if(recyView instanceof RecyclerView){
  ...
  /*  private View mScrollView;
    ...
    protected boolean isReadyForPullStart() {//return true;
        if(mIsExpland){
            if(isRecyclerViewTop(mCurrentScrollableCainer.getScrollableView())) return true;
            return false;
    ...
    public final boolean onTouchEvent(MotionEvent event) {
        ...
        LogUtils.e("GestureFrameLayout", "onTouchEvent_"+ getActionName(event.getAction()));
        //if(!mIsExpland)  return false;
        if(mIsPlaying)   return  false;
        if(mIsLandScape) return  false;
            ...
            case MotionEvent.ACTION_UP: {
               ...
               /*      if (mState == State.RELEASE_TO_REFRESH
                    ...
                    //当滑动的距离大于我们设定的最小距离且滑动的瞬间速度大于我们设定的速度时，返回到上一个activity
                     ...
                     if(((getScrollY())!=0&&Math.abs(distanceY) > mTouchSlop) || (((isExpland()&&(distanceY>0))||(!isExpland()&&(distanceY<0)))&&(xSpeed > XSPEED_MIN))){//distanceY<0&&
                   ...
                   /* if(((isExpland()&&(getScrollY()!=0))||distanceY > mTouchSlop || (distanceY>0&&(xSpeed > XSPEED_MIN)))) {
                        //finish();
                        boolean oldFlag=isExpland();
                        switchExpland();
                        if(null!=mOnPulltoUpListener) mOnPulltoUpListener.onPulltoUpEvent(!oldFlag);
                    }
                    else if((!isExpland()&&(getScrollY()!=mTopDistance))||(distanceY<0&&(Math.abs(distanceY) > mTouchSlop)) ||(distanceY<0&&(xSpeed > XSPEED_MIN))){//||(distanceY<0&&(xSpeed > XSPEED_MIN)
                        switchExpland();
                        if(null!=mOnPulltoUpListener) mOnPulltoUpListener.onPulltoUpEvent(true);
