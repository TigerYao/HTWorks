...
/**   参考com.cyl.musiclake项目
 * 作者：yonglong on 2016/8/11 19:16
 * 邮箱：643872807@qq.com
 * 版本：2.5 播放service
 */
public class MusicPlayerService extends Service implements IMediaPlayer.OnUpdatePlayPositionListener  {
    ...
    public static final String CMD_CLEAR = "clear_stop";// 自定义行为  清除播放列表，通知栏，停止播放器
    ...
    public static final int PREPARE_ASYNC_UPDATE = 7; //PrepareAsync装载进程  缓存的进度
    ...
    public static final int CATON_PRERPAREING = 15; //自定义行为 卡顿正在缓存
    public static final int CATON_PRERPARED = 16;   //自定义行为 卡顿 缓存完
  ...
  //  private MusicPlayerEngine mPlayer = null;
    ...
    private  MusicPlayerBJEngine mPlayer = null;
...
/*    private Disposable disposable = Observable
   ...
   // private Subscription mObservableOfTimer;

    private void unSubscribeTimer() {
      /*  if(this.mObservableOfTimer != null && !this.mObservableOfTimer.isUnsubscribed()) {
            this.mObservableOfTimer.unsubscribe();
        }
         this.mObservableOfTimer = null;*/
    }

/*
    private Subscription mObservableOfTimer = Observable.interval(1000, TimeUnit.MILLISECONDS)
            .observeOn(AndroidSchedulers.mainThread()).subscribe(new com.baijiahulian.player.b.a<Long>() {
                 @Override
...
*/
    ...
    private void startIntervalTimer(){
       /* if(mObservableOfTimer==null){
            mObservableOfTimer=Observable.interval(1000, TimeUnit.MILLISECONDS)
                    .observeOn(AndroidSchedulers.mainThread()).subscribe(new com.baijiahulian.player.b.a<Long>() {
                        @Override
                        public   void call(Long var1){
                            for (int i = 0; i < listenerList.size(); i++) {
...
//                        service.bumpSongCount(); //更新歌曲的播放次数
                    ...
                    case TRACK_PLAY_ENDED://mPlayer播放完毕且暂时没有下一首
                         if (false) {//PlayQueueManager.INSTANCE.getPlayModeId() == PlayQueueManager.PLAY_MODE_REPEAT
                    ...
                    case PREPARE_ASYNC_UPDATE:
                        ...
                        break;
                    case CATON_PRERPAREING:
                        LogUtil.e(TAG, "CATON_PRERPAREING");
                        notifyWaiting(true);
                        break;
                    case CATON_PRERPARED:
                        LogUtil.e(TAG, "CATON_PRERPARED");
                        notifyWaiting(false);
                        break;
    ...
    /**
     * 重新加载当前进度  用来加载最后播放的数据，暂时不需要
     */
    private void reloadPlayQueue() {

        PlayQueueManager.INSTANCE.clear();
       // mPlayQueue =new ArrayList<>();// PlayQueueLoader.INSTANCE.getPlayQueue();
        /*mPlayingPos =0;// SPUtils.getPlayPosition();
    ...
    private void initMediaPlayer() {
       // mPlayer = new MusicPlayerEngine(this);
        ...
        mPlayer = new MusicPlayerBJEngine(this);
        mPlayer.setPlayPostionListener(this);//IJKMediaPlayer 已实现Timer
...
//                mShutdownScheduled = true;
 ...
 /*   private void setAndRecordPlayPos(int mNextPlayPos) {
    ...
    public void setPlayRate(float rateSpeed){
        if (mPlayer != null) {
              mPlayer.setPlayRate(rateSpeed);
        }
    }

    public float getPlayRate(){
        if (mPlayer != null) {
           return mPlayer.getPlayRate();
        }
        return 1f;
    }
    ...
    public void setLoopMode(int loopmode){
        ...
        if(PlayQueueManager.INSTANCE.getPlayModeId()==PlayQueueManager.PLAY_MODE_LOOP){
            PlayQueueManager.INSTANCE.updatePlayMode(PlayQueueManager.PLAY_MODE_REPEAT);
        }else {
     ...
     * 下一首
     */
    public void next(Boolean isAuto,int amount) {
        synchronized (this) {
            int nextPlayPos=PlayQueueManager.INSTANCE.skipQueuePosition(isAuto,amount);
            if(nextPlayPos==-1){
                pause();
                return;
            }
            if ((!PlayQueueManager.INSTANCE.isLocalPath(nextPlayPos))
                    &&(!NetworkUtils.isNetworkAvailable(this))) {
                showToast("网络不可用，请检查网络连接");
                return;
            }
           // setPlayRate(1);
           // mPlayingPos = nextPlayPos;
            LogUtil.e(TAG, "next: " + nextPlayPos);
            stop(false);
            playCurrentAndNext();
        }
    }
     ...
     * 播放当前歌曲
     */
    private void playCurrentAndNext(boolean isFirst) {
        synchronized (this) {
            if (PlayQueueManager.INSTANCE.isOutOfPlayList()) {
                return;
            }
            Music mPlayingMusic = PlayQueueManager.INSTANCE.getCurrentItem();
            //更新当前歌曲
            notifyChange(META_CHANGED);
            updateNotification(true);
            //更新播放播放状态
            ...
            PlayQueueManager.INSTANCE.saveHistory(getCurrentPosition());

            if(!TextUtils.isEmpty(mPlayingMusic.uri)){
                if (!mPlayingMusic.uri.startsWith(Constants.IS_URL_HEADER) &&(!SystemUtils.exists(mPlayingMusic.uri)) ) {//!FileUtils.exists(mPlayingMusic.getUri())
                    checkPlayErrorTimes();
                } else {
                    playErrorTimes = 0;
                    mPlayer.setDataSource(mPlayingMusic.uri,isFirst ? mPlayingMusic.trackNumber  :0);
                }
           }
            else if(mPlayingMusic.id>0&&(!TextUtils.isEmpty(mPlayingMusic.collectId))){
                playErrorTimes = 0;
                mPlayer.setVideoInfo(mPlayingMusic.id,mPlayingMusic.collectId,isFirst ? mPlayingMusic.trackNumber  :0);
            }
...
//                notifyChange(PLAY_STATE_CHANGED);
     ...
     * @param position
     */
    public void playMusic(int position,int startPos) {
        boolean isSucc= PlayQueueManager.INSTANCE.resetplayMusic(position,startPos);
        if(isSucc){
           mMediaSessionManager.canSessionActive(true);
           stop(false);
           playCurrentAndNext(true);
        }
    }
    ...
    public void play(Music music) {
        ...
        PlayQueueManager.INSTANCE.addAndplayMusic(music);
        setPlayRate(1);
        //发送播放列表改变
        notifyChange(PLAY_QUEUE_CHANGE);
    ...
    /**  change by cjx 复用此方法实现从尾部添加 与从顶部添加表   id -1从头部添加，-2从尾部添加
     * 切换歌单播放
     * 1、歌单不一样切换，不一样不切换
     * 2、相同歌单只切换歌曲
     * 3、相同歌曲不重新播放
     ...
     * @param pid       歌单id
     */
    public void play(List<Music> musicList, int index, String pid) {
        LogUtil.d(TAG, "musicList = " + musicList.size() + " id = " + index + " pid = " + pid + " mPlaylistId =" + PlayQueueManager.INSTANCE.getPlayListId());

        if (musicList.size() <= index) return;
        if (PlayQueueManager.INSTANCE.isSamePlayingPos(index,pid)) return;
        PlayQueueManager.INSTANCE.resetOrAddPlayQueue(musicList,index,pid);
    ...
    public void pause() {
        ...
        synchronized (this) {
            ...
            if (isPlaying()) {
               // mediaSessionManager.canSessionActive(false);
                unSubscribeTimer();
                isMusicPlaying = false;
                notifyChange(PLAY_STATE_CHANGED);
               ...
               /* TimerTask task = new TimerTask() {
                ...
                mMainHandler.postDelayed(task,200);
            }
        }
    }
    ...
    public void seekTo(long pos, boolean isInit) {
        ...
        Music playingMusic=PlayQueueManager.INSTANCE.getCurrentItem();
        if (mPlayer != null && mPlayer.isInitialized() && playingMusic != null) {
            mPlayer.seek(pos);
...
//            mPlayer.pause();
    ...
    public boolean onUnbind(Intent intent) {
        ...
        mServiceInUse = false;
        PlayQueueManager.INSTANCE.savePlayQueue(false);

        //注掉代码让所有的客户端解绑定之后继续运行
       /* releaseServiceUiAndStop();
    ...
    public void clearQueue() {
        PlayQueueManager.INSTANCE.clear();
        isMusicPlaying = false;
        PlayQueueManager.INSTANCE.savePlayQueue(true);
        stop(true);
        notifyChange(META_CHANGED);
        notifyChange(PLAY_STATE_CHANGED);
    ...
    //通知Activity正在缓存中
    private void notifyWaiting(boolean isWaiting){
        //updateWidget(ACTION_PLAY_PAUSE);
        //mediaSessionManager.updatePlaybackState();
        EventBus.getDefault().post(new StatusChangedEvent(isPrepared(), isPlaying(),isWaiting?1:2));
    }
    /**
     * 发送更新广播
     *
...
//                notifyChange(PLAY_STATE_CHANGED);
                Music playingMusic=PlayQueueManager.INSTANCE.getCurrentItem();
                EventBus.getDefault().post(new MetaChangedEvent(playingMusic));
                break;
...
//                break;
    ...
    private void updateWidget(String action) {
        ...
        if (action.equals(META_CHANGED)) {
            intent.putExtra(Extras.SONG, playingMusic);
        }
        sendBroadcast(intent);
    }
    ...
    public String getArtistName() {
        Music playingMusic=PlayQueueManager.INSTANCE.getCurrentItem();
        if (playingMusic != null) {
            return playingMusic.artist;//.getArtist();
//            return ConvertUtils.getArtistAndAlbum(mPlayingMusic.getArtist(), mPlayingMusic.getAlbum());
    ...
    public List<Music> getPlayQueue() {
         return PlayQueueManager.INSTANCE.getPlayQueue();
    }
    ...
    public int getPlayPosition() {
        int playIndex=PlayQueueManager.INSTANCE.getCurPlayPositon();
        if (playIndex >= 0) {
            return playIndex;
        } else return 0;
    }
    ...
    private void initNotify() {
      ...
      //  Intent nowPlayingIntent = new Intent(this, PlayerActivity.class);
        ...
        nowPlayingIntent.setComponent(new ComponentName(this,Constants.OPEN_NOTICE_URL));
        nowPlayingIntent.setAction(Constants.DEAULT_NOTIFICATION);
        nowPlayingIntent.putExtra("fromNotice",true);
        PendingIntent clickIntent = PendingIntent.getActivity(this, 0, nowPlayingIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        if (mNotificationPostTime == 0) {
            mNotificationPostTime = System.currentTimeMillis();
        }

        PendingIntent stopPendIntent=null;
        try{
            stopPendIntent=  MediaButtonReceiver.buildMediaButtonPendingIntent(
                    this, PlaybackStateCompat.ACTION_STOP);
        }catch (Exception e){
            ComponentName  buttonRecComponent = new ComponentName(this.getPackageName(),
                    MediaButtonIntentReceiver.class.getName());
            stopPendIntent= MediaButtonReceiver.buildMediaButtonPendingIntent(this,
                             buttonRecComponent,PlaybackStateCompat.ACTION_STOP);
        }
        ...
        mNotificationBuilder = new Builder(this, initChannelId())
           ...
           if(null!=stopPendIntent) {
               mNotificationBuilder.setDeleteIntent(stopPendIntent);
          }
        ...
        if (SystemUtils.isLollipop()) {
            ...
            NotificationCompat.MediaStyle style = new NotificationCompat.MediaStyle()
                    .setMediaSession(mMediaSessionManager.getMediaSession())
        ...
        Music playingMusic=PlayQueueManager.INSTANCE.getCurrentItem();
        if (playingMusic != null) {
            CoverLoader.loadImageViewByMusic(this, playingMusic, new CoverLoader.Callback<Bitmap>() {
                 @Override
    ...
    private String initChannelId() {
        // 通知渠道的id
        String id = "music_huatu_01";
        // 用户可以看到的通知渠道的名字.
        CharSequence name = "华图在线";
        // 用户可以看到的通知渠道的描述
    ...
    public String getAudioId() {
        Music playMusic=PlayQueueManager.INSTANCE.getCurrentItem();
        if (playMusic != null) {
            return playMusic.mid;//.getMid();
        } else {
            return null;
        }
    }
    ...
    private void handleCommandIntent(Intent intent) {
        ...
        if (CMD_NEXT.equals(command) || ACTION_NEXT.equals(action)) {
           ...
           // mFloatLyricViewManager.saveLock(false, true);
            ...
            isMusicPlaying = false;
            mMediaSessionManager.canSessionActive(false);
            mMediaSessionManager.updatePlaybackState();
            PlayQueueManager.INSTANCE.clear();
