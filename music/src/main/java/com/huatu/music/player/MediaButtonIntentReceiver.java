...
public class MediaButtonIntentReceiver extends BroadcastReceiver {
            ...
            switch (msg.what) {
                case MSG_LONGPRESS_TIMEOUT:
                    ...
                    if (!mLaunched) {
                        ...
                        i.setComponent(new ComponentName(context.getPackageName(),"com.huatu.handheld_huatu.business.guide.SplashActivity"));
                       // i.setClass(context, MainActivity.class);
    ...
    private static void startService(Context context, String command) {
        ...
        //播放列表为空的时，不响应操作
        if(PlayQueueManager.INSTANCE.getSize()<=0){
            return;
        }
            ...
            if (command != null) {
                if (action == KeyEvent.ACTION_DOWN) {
                    if (mDown) {
                        ...
                        if (keycode == KeyEvent.KEYCODE_HEADSETHOOK) {//耳机单按键的按键或三按键耳机的中间按键。与上面的按键可能是相同的，具体得看驱动定义。
